...
describe('10 - Implemente os casos de teste e a função `createMenu`', () => {
  it('Verifica se a função `createMenu` tem o comportamento esperado', () => {
    ...
    expect(Object.keys(createMenu({ food: {}, drink: {} }).fetchMenu()).length).toBe(2);
    ...
    expect(createMenu('teste').fetchMenu()).toEqual('teste');
    ...
    const tudo = createMenu();
    tudo.order('teste');
    expect(tudo.consumption).toEqual(["teste"]);
    tudo.consumption = [];
    // const objetoRetornado = createMenu(objetoQualquer);
    ...
    tudo.order('agua')
    tudo.order('sopa')
    expect(tudo.consumption).toEqual(["coxinha", "agua", "sopa"]);
    tudo.consumption = [];
    // objetoRetornado.order("coxinha");
    ...
    tudo.order('coxinha');
    tudo.order('coxinha');
    expect(tudo.consumption).toEqual(["coxinha", "coxinha"]);
    tudo.consumption= [];
    // objetoRetornado.order('coxinha');
    // objetoRetornado.order('agua');
    // objetoRetornado.order('coxinha');
    ...
    // TESTE 8: Verifique se, ao chamar `objetoRetornado.pay()`, retorna-se a soma dos preços de tudo que foi pedido, conforme registrado em `objetoRetornado.consumption`
        tudo.order({ food: {'coxinha': 3.9, 'sopa': 9.9}, drink: {'agua': 3.9, 'cerveja': 6.9} });
    
    

    expect(tudo.pay()).toBeCloseTo(27.06); 
    // objetoRetornado.order('coxinha');
    // objetoRetornado.order('agua');
    // objetoRetornado.order('coxinha');
