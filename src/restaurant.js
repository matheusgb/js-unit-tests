...
const createMenu = (parametro) => {
  ...
  consumption: [],
  order: (parametro2) => {
  const menuzin = menu.consumption;
  menuzin.push(parametro2);
  },
  fetchMenu: () => parametro, 

  pay: () => {
    let soma = 0;
    const foods = Object.values(menu.consumption[0].food);
    const drinks = Object.values(menu.consumption[0].drink);
